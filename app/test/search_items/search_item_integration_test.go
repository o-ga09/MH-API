...
func TestGetItem(t *testing.T) {
	...
	cases := []struct {
		...
		// {name: "ItemNameKanaを指定して取得できる", path: "/v1/items=カイフクヤク", expected_status: 200, expected_body: expectedItem5},
		// {name: "取得結果が0件の場合、404になる", path: "/v1/items?itemIds=1111111111,2222222222,3333333333", expected_status: 404, expected_body: `{"item":"expected item"}`},
		// {name: "どこかでエラーの場合、500になる", path: "/v1/items", expected_status: 500, expected_body: `{"item":"expected item"}`},
	}
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.expected_body.Limit, data.Limit)
			assert.Equal(t, tt.expected_body.Offset, data.Offset)
			assert.Equal(t, tt.expected_body.Total, data.Total)
			assert.Equal(t, tt.expected_body.Item, data.Item)
		})
	}
}
